...
__all__ = (
    ...
    'any_of', 'NoneOf', 'none_of', 'MacAddress', 'mac_address', 'UUID','PasswordCheck','password_check'
)
...
class PasswordCheck(object):
    """
    Ensure password contains lowercase, uppercase and digit and list of other digits
    """
    def __init__(self, message=None, min=10, special_characters=None):
    ...
    def __call__(self, form, field):
        if self.special_characters:
            if len(field.data) >= self.min and any(char.isdigit() for char in field.data) and any(char.islower() for char in field.data) and any(char.isupper() for char in field.data) and any(char in self.special_characters for char in field.data):
                return True
            else:
                raise ValidationError(self.message)
        else:
            if len(field.data) >= self.min and any(char.isdigit() for char in field.data) and any(char.islower() for char in field.data) and any(char.isupper() for char in field.data):
                return True
            else:
                raise ValidationError(self.message)

# class TwosaCrowd(object):
#     """
#     Ensures there are either of these two fields. E.G One of either must have input
...
#             message = self.message
#             if message is None:
#                 message = field.gettext('Must only have one of these fields: %(other_name)s.')

#             raise ValidationError(message % d)


# twosacrowd = TwosaCrowd
